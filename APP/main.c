...
int main (void){
	...
	LED_Config	LOC_strWarning_LED = {PORTD ,PIN6 ,ACTIVE_HIGH};
					...
					if( LOC_u8Flag == PASS_WORD_CHAR_NUM )
					{
						...
						while(LOC_u8Flag==0)
						{
							...
							}else if(LOC_u8Pressed_Button == CHANGE_PASSWORD_OP )
							{
								...
								CLCD_voidSetPosition(CLCD_ROW_2,CLCD_COL_1);
								for(LOC_u8Iterator=0;LOC_u8Iterator<PASS_WORD_CHAR_NUM;LOC_u8Iterator++)
								{
									LOC_u8Pressed_Button = NOT_PRESSED;
									/*wait until press any button to turn on the CLCD*/
									while(LOC_u8Pressed_Button == NOT_PRESSED){LOC_u8Pressed_Button = KEY_PAD_ptru8GET_PRESSED_VALUE();}
									LOC_ptru8Pass_word_Check[LOC_u8Iterator] = LOC_u8Pressed_Button;
									CLCD_voidSend_Data('*');
								}	
								Clear_voidCLCD();
								CLCD_voidSetPosition(CLCD_ROW_1,CLCD_COL_1);
								...
								for(LOC_u8Iterator=0;LOC_u8Iterator<PASS_WORD_CHAR_NUM;LOC_u8Iterator++)
								{
									...
									/*wait until press any button to turn on the CLCD*/
									while(LOC_u8Pressed_Button == NOT_PRESSED){LOC_u8Pressed_Button = KEY_PAD_ptru8GET_PRESSED_VALUE();}
									if( LOC_ptru8Pass_word_Check[LOC_u8Iterator] == LOC_u8Pressed_Button)
									{
										LOC_u8Flag++;
									}
									CLCD_voidSend_Data('*');
								...
								if(LOC_u8Flag==4)
								{
									...
									for(LOC_u8Iterator = 0;LOC_u8Iterator<PASS_WORD_CHAR_NUM;LOC_u8Iterator++)
									{
										LOC_ptru8PassWord[LOC_u8Iterator] = LOC_ptru8Pass_word_Check[LOC_u8Iterator]; 
									}
